...
serve(async (req) => {
  ...
  try {
    ...
    // Get responder's OneSignal player ID from onesignal_subscriptions table
    ...
    if (subsError) {
      console.warn(`Error fetching OneSignal subscriptions for responder ${responder.name}:`, subsError)
    }

    if (!subscriptions || subscriptions.length === 0 || !subscriptions[0]?.player_id) {
      console.warn(`No OneSignal player ID found for responder ${responder.name} (user_id: ${responder.user_id})`)
      return new Response(
        JSON.stringify({ 
          success: true, 
          sent: 0, 
    ...
    const playerIdData = subscriptions[0]
    console.log(`✅ Found OneSignal player ID for ${responder.name}:`, playerIdData.player_id)
